...
public:
	...
	struct npc_glintrok_oracleAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff)
		{
			...
			if (me->IsWithinMeleeRange(me->ToCreature(), 2.5f) && !isWithinMeleeRange)
			{
				...
				switch(eventId)
				{
					if (instance)
					{
						...
						case EVENT_FIRE_BOLT:
							me->CastSpell(me->ToCreature(), SPELL_FIRE_BOLT);
				...
				switch(eventId)
				{
					if (instance)
					{
						case EVENT_HEX_OF_LETHARGY:
							me->CastSpell(me->ToCreature(), SPELL_HEX_OF_LETHARGY);

							events.ScheduleEvent(EVENT_HEX_OF_LETHARGY, 10*IN_MILLISECONDS, 0);
							break;

						case EVENT_DARK_BOLT:
							me->CastSpell(me->ToCreature(), SPELL_DARK_BOLT);
...
public:
	...
	struct npc_glintrok_skulkerAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff)
		{
			...
			{
				me->CastSpell(me->ToCreature(), SPELL_SHANK);
