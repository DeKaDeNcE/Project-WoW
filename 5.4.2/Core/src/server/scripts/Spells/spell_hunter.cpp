...
public :
    ...
    class spell_hun_chimera_shot_SpellScript : public SpellScript
    {
        ...
        {
            sLog->outDebug("network", "Chimera Shot : handleMiscOnEffectScriptEffect");
            if(GetCaster())
            {
                sLog->outDebug("network", "Chimera Shot : caster as unit is not null");
                if(Player* caster = GetCaster()->ToPlayer())
                {
                    // Reset Serpent Sting duration && apply Mark of the Hunter
                    sLog->outDebug("network", "Chimera Shot : caster as player is not null");
                    if(Unit* target = GetExplTargetUnit())
                    {
                        sLog->outDebug("network", "Chimera Shot : target is not null");
                        if(Aura* serpentSting = target->GetAura(118253))
                        {
                            sLog->outDebug("network", "Chimera Shot : target has aura serpent sting ; refreshing it");
                            serpentSting->RefreshDuration();
                        }

                        sLog->outDebug("network", "Chimera Shot : casting mark of the hunter");
                        // Mark of the Hunter
                        caster->CastSpell(target, 1130, TRIGGERED_FULL_MASK);
                    }

                    // Heal the caster
                    sLog->outDebug("network", "Chimera Shot : calculating hp");
                    ...
                    sLog->outDebug("network", "Chimera Shot : life going to be set to %f", healthToRestore);
