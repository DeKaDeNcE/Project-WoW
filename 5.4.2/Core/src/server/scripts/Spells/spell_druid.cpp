				...
				switch(event)
				{
					...
					case EVENT_WRATH:
					{
						if(Player* owner = ((TempSummon*)me)->GetSummoner()->ToPlayer())
						{
							...
							me->CastCustomSpell(me->ToCreature(), SPELL_WRATH, &bp0, NULL, NULL, false);
						}
...
public :
    ...
    class spell_dru_frenzied_regeneration_SpellScript : public SpellScript
    {
        ...
        {
            sLog->outDebug("network", "Frenzied Regeneration : handleHealOnEffectDummy");
            if(Unit* caster = GetCaster())
            {
                sLog->outDebug("network", "Frenzied Regeneration : caster as unit is not null");
                if(Player* p = GetCaster()->ToPlayer())
                {
                    sLog->outDebug("network", "Frenzied Regeneration : caster as player is not null");
                    if(p->HasAura(54810))
                        return ;

                    sLog->outDebug("network", "Frenzied Regeneration : caster has not the glyph, calculating amounts");
        ...
        {
            sLog->outDebug("network", "Frenzied Regeneration : handleModHealingDoneOnEffectDummy");
            if(GetCaster())
            {
                sLog->outDebug("network", "Frenzied Regeneration : caster as unit not null");
                if(Player* caster = GetCaster()->ToPlayer())
                {
                    sLog->outDebug("network", "Frenzied Regeneration : caster as player not null ; casting the heal");
                    ...
                    {
                        sLog->outDebug("network", "Frenzied Regeneration : caster has the aura ; checking glyph");
                        if(caster->HasAura(54810))
                        {
                            sLog->outDebug("network",
