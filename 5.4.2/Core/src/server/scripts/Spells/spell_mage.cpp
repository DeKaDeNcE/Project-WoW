    ...
    class spell_mage_alter_time_AuraScript : public AuraScript
    {
        ...
        void handleSavePlayerOnEffectApply(AuraEffect const* auraEff, AuraEffectHandleModes mode)
        {
			...
			if(p_caster)
			{
				...
				sLog->outDebug("network", "Applying Alter Time ; saving player");
		...
		void handleResetPlayerOnEffectRemove(AuraEffect const* auraEff, AuraEffectHandleModes mode)
		{
			...
			if(p_caster)
			{
				...
				sLog->outDebug("network", "Removing Alter Time ; player teleporting");
				p_caster->TeleportTo(m_savedPos, 0);
				
				sLog->outDebug("network", "Removing Alter Time ; auras resetting");
				if(GetCaster())
				{
					sLog->outDebug("network", "Caster as Unit is not null");
					if(GetCaster()->ToPlayer())
					{
						sLog->outDebug("network", "Caster as player is not null ; resetting !");
...
public :
	...
	class spell_mage_combustion_SpellScript : public SpellScript
	{
		...
		{
			sLog->outDebug("network", "Entering Combusion OnCast Handler");
			if(GetCaster())
			{
				sLog->outDebug("network", "Combustion's Caster as unit is not null");
				if(Player* p = GetCaster()->ToPlayer())
				{
					sLog->outDebug("network", "Combustion's Caster as player is not null ; resetting cooldown");
                    ...
                    for(SpellCooldowns::iterator iter = cooldowns.begin() ; iter != cooldowns.end() ; ++iter)
                    {
                        ...
                        if(!spell)
                            continue ;
                        sLog->outDebug("network", "Player %s (guid : %u) has spell cooldown on spell %s (id : %u)", p->GetName().c_str(), p->GetGUIDLow(), spell->SpellName, spell->Id);
                    }
		...
		{
			sLog->outDebug("network", "Entering Combusion OnEffectHitTarget Handler");
			if(Unit* target = GetExplTargetUnit())
			{
				// Combusion impact = stun
				if(GetCaster())
				{
					sLog->outDebug("network", "Combustion's Target not null ; ready to impact");
				...
				{
					sLog->outDebug("network", "Combustion's Target has Ignite aura ; ready to apply the dot");
					if(GetCaster() && target)
					{
						...
						{
							sLog->outDebug("network", "Applying the dot on Combusion target");
...
public :
    ...
    class spell_mage_inferno_blast_SpellScript : public SpellScript
    {
        ...
        {
            sLog->outDebug("network", "Inferno Blast : OnHit Handler");
            if(GetExplTargetUnit() && GetCaster() && GetCaster()->ToPlayer())
            {
                sLog->outDebug("network", "Inferno Blast : ExplicitTarget found (name %s, guid %u) ; casting Inferno Blast !", GetExplTargetUnit()->GetName().c_str(), GetExplTargetUnit()->GetGUID());
...
public :
    ...
    class spell_mage_inferno_blast_spreader_SpellScript : public SpellScript
    {
        ...
        {
            sLog->outDebug("network", "Inferno Blast Spreader : OnObjectAreaTargetSelect Handler");
            if(Unit* caster = GetCaster())
            {
                Unit* realTarget = GetExplTargetUnit();
                sLog->outDebug("network", "Inferno Blast Spreader : Checking real target");
                if(!realTarget)
                    return ;
				...
				sLog->outDebug("network", "Inferno Blast Spreader : real target found (name %s, guid %u) ; position set ; checking glyph", realTarget->GetName().c_str(), realTarget->GetGUID());
                // Glyph of Discret Magic : target is valid only if less than 5 yards away from the PRIMARY TARGET
                if(caster->HasAura(134580))
                {
                    sLog->outDebug("network", "Inferno Blast Spreader : caster has glyph ; checking availables targets");
                ...
                sLog->outDebug("network", "Inferno Blast Spreader : glyph test done ; resizing list (actual size : %u)", (uint32)targets.size());
        ...
        {
            sLog->outDebug("network", "Inferno Blast Spreader : OnEffectHitTarget Handler");
            ...
            {
                sLog->outDebug("network", "Inferno Blast Spreader : pointers corrects");
                if(realTarget->GetGUID() == otherTarget->GetGUID()) // This would be tricky
                    return ;

                sLog->outDebug("network", "Inferno Blast Spreader : guids corrects (realTarget : %u ; otherTarget : %u)", realTarget->GetGUID(), otherTarget->GetGUID());
				Unit::AuraApplicationMap auras = realTarget->GetAppliedAuras(); // To find the auras to duplicate
                for(Unit::AuraApplicationMap::iterator iter = auras.begin() ; iter != auras.end() ; ++iter)
                {
					sLog->outDebug("network", "Inferno Blast Spreader : looping");
					if(!iter->second)
						continue;

					if(Aura* actualAura = iter->second->GetBase())
                    {
						sLog->outDebug("network", "Inferno Blast Spreader : found an aura (%s, %u) ; casterGUID = %u", actualAura->GetSpellInfo()->SpellName, actualAura->GetId(), actualAura->GetCasterGUID());
                        if(actualAura->GetCaster() == caster)
                        {
                            sLog->outDebug("network", "Inferno Blast Spreader : aura (%s, %u) found on realTarget", actualAura->GetSpellInfo()->SpellName, actualAura->GetId());
                            Aura* copy ;
                            switch(actualAura->GetId())
                            {
                            case 12654 : case 11366 : case 44457 : case 118271 :
                                // Well, thanks Pexirn. A LOT !
                                sLog->outDebug("network", "Inferno Blast Spreader : good aura (%s, %u) found on realTarget", actualAura->GetSpellInfo()->SpellName, actualAura->GetId());
                            ...
                            {
                                // Thanks Pexirn. Again.
                                sLog->outDebug("network", "Inferno Blast Spreader : copy aura has been created ; setting target in combat if it wasn't");
                                if(!otherTarget->IsInCombat())
                                    otherTarget->SetInCombatWith(caster);

                                // One last time : THANKS PEXIRN ! (not fot the headache you bitch)
                                sLog->outDebug("network", "Inferno Blast Spreader : enterring the twisted part (well, i hope not in fact)");
                                if(AuraApplication* auraApp = copy->GetApplicationOfTarget(caster->GetGUID()))
                                {
                                    sLog->outDebug("network", "Inferno Blast Spreader : we entered the twisted part. Good Lord, please, HEEEEEEEEEELP ME !");
...
public :
	...
	/*class spell_mage_evocation_SpellScript : public SpellScript
	{
		...
		{
			sLog->outDebug("network", "Evocation : Entering OnEffectHitTarget (effectIndex = 2) Handler");
			if(GetCaster() && GetCaster()->ToPlayer())
			{
				sLog->outDebug("network", "Evocation : Caster is not null and is a player ; calculating 15% of totalMana ; totalMana = %u", GetCaster()->GetMaxPower(POWER_MANA));
				int32 totalMana = GetCaster()->GetMaxPower(POWER_MANA);
				totalMana /= 100 ;
				totalMana *= 15 ;
				
				sLog->outDebug("network", "Evocation : amount calculated : expecting %u mana points", totalMana);
	...
	class spell_mage_evocation_AuraScript : public AuraScript
	{
		...
		{
			sLog->outDebug("network", "Evocation (Aura) : Entering OnEffectHitTarget (effectIndex = 1) Handler");
			// Do not apply heal if player hasn't the glyph
			if(GetCaster() && GetCaster()->ToPlayer())
			{
				sLog->outDebug("network", "Evocation (Aura) : caster is not null and is a player");
				if(!GetCaster()->HasAura(56380))
				{
					sLog->outDebug("network", "Evocation (Aura) : caster doesn't have the glyph ; preventing default effect (apply aura : heal)");
					PreventDefaultAction();
				}
			}
		}
...
public :
	...
	class spell_mage_time_warp_SpellScript : public SpellScript
	{
		...
		void filterTargets(std::list<WorldObject*>& targets)
		{
			sLog->outDebug("network", "Time Warp : Entering OnObjectAreaTargetSelect handler ; number of targets : %u", uint32(targets.size()));
		...
		{
			sLog->outDebug("network", "Time Warp : Entering AfterHit Handler");
			if(GetHitUnit())
			{
				sLog->outDebug("network", "Time Warp : GetHitUnit() not null ; applying aura");
