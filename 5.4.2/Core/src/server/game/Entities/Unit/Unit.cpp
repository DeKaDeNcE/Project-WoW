    ...
    switch (mtype)
    {
        ...
        case MOVE_FLIGHT:
        {
            ...
            if (GetTypeId() == TYPEID_UNIT)
            {
                ...
                if ((IsPet() || IsGardian()) && !IsInCombat() && pOwner) // Must check for owner or crash on "Tame Beast"
                {
...
//        ASSERT(false);
    ...
    else if (GetTypeId() == TYPEID_PLAYER || (GetTypeId() == TYPEID_UNIT && !ToCreature()->IsGardian()))
        DeleteCharmInfo();
}
