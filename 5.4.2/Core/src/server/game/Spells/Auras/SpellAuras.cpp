...
void Aura::SendFakeAuraUpdate(uint32 auraId, bool remove)
{
    ...
    if(remove)
    {
        GetCaster()->SendMessageToSet(&data, true);
        return;
    }

    uint8 flags = AFLAG_POSITIVE;

    if (GetMaxDuration() > 0 && !(GetSpellProto()->AttributesEx5 & SPELL_ATTR5_HIDE_DURATION))
        flags |= AFLAG_DURATION;

    data << uint8(flags);
    data << uint8(GetCasterLevel());
    data << uint8(GetStackAmount() > 1 ? GetStackAmount() : (GetCharges()) ? GetCharges() : 1);
