...
void Player::HandleDrowning(uint32 time_diff)
{
    ...
    if (m_MirrorTimerFlags & UNDERWATER_INWATER)
    {
        ...
        if (m_areaUpdateId == 5711) return;
        if (m_areaUpdateId == 5059) return;
        if (m_areaUpdateId == 5055) return;
        if (m_areaUpdateId == 5007) return;
...
void Player::GiveXP(uint32 xp, Unit *victim, float group_rate)
{
    ...
    float exp_mult = 1.0f;
    if (map == 638)
        exp_mult = 0.27f; //you get 37x more XP, so you want divide it by 37, so 1/37 ~> 0.027
    xp = uint32(xp * exp_mult);
